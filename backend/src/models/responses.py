...
class IndicadorEconomico(BaseModel):
    ...
    valor_indicador: float = Field(
        description="Valor do indicador em percentual anual"
    )
    fonte: str = Field(
        description="Fonte dos dados (ex: Banco Central, IBGE)"
    )
    data_referencia: str = Field(
        description="Data de referência dos dados no formato YYYY-MM-DD"
    )
...
class TaxasAplicadas(BaseModel):
    ...
    formula_aplicada: str = Field(
        description="Fórmula utilizada no cálculo da taxa"
    )
...
class ResultadoFinanciamento(BaseModel):
    ...
    total_pago: float = Field(
        description="Valor total a ser pago ao final do financiamento"
    )
    juros_totais: float = Field(
        description="Valor total de juros pagos"
    )
    percentual_juros: float = Field(
        description="Percentual que os juros representam do total pago"
    )
    primeira_parcela: DetalheParcela = Field(
        description="Detalhes da primeira parcela"
    )
    ultima_parcela: DetalheParcela = Field(
        description="Detalhes da última parcela"
    )
...
class Comparativo(BaseModel):
    ...
    mensagem: str = Field(
        description="Mensagem explicativa sobre o comparativo"
    )
...
class Analise(BaseModel):
    ...
    alertas: List[str] = Field(
        ...
        description="Lista de alertas e recomendações"
    )
...
class SimulationResponse(BaseModel):
    ...
    timestamp: datetime = Field(
        ...
        description="Data e hora da simulação"
    )
